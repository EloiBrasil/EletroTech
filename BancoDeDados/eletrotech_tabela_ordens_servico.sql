...
CREATE TABLE `tabela_ordens_servico` (
  `id_ordServ` int NOT NULL AUTO_INCREMENT,
  `eletricista_os` int DEFAULT NULL,
  `data_os` date DEFAULT NULL,
  PRIMARY KEY (`id_ordServ`),
  KEY `eletricista_os` (`eletricista_os`),
  CONSTRAINT `tabela_ordens_servico_ibfk_1` FOREIGN KEY (`eletricista_os`) REFERENCES `tabela_eletricistas` (`id_eletri`)
